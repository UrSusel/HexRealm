...
try {
    ...
    // Woda na brzegach (mocniej na dole mapy)
    for ($y = 0; $y < $height; $y++) {
        for ($x = 0; $x < $width; $x++) {
            ...
            if ($d === 0) {
                $tiles[$y][$x] = 'water';
            } elseif ($d === 1 && rand(1, 100) <= ($bottomBand ? 85 : 65)) {
                $tiles[$y][$x] = 'water';
            } elseif ($d === 2 && rand(1, 100) <= ($bottomBand ? 55 : 35)) {
                $tiles[$y][$x] = 'water';
            }
            if ($bottomBand && $d === 3 && rand(1, 100) <= 35) {
                $tiles[$y][$x] = 'water';
            }
        }
    }
    ...
    // Skupiska lasu
    $forestSeeds = max(5, (int)round(($width * $height) / 320));
    ...
    $grass2Seeds = max(5, (int)round(($width * $height) / 300));
    ...
    for ($y = 0; $y < $height; $y++) {
        for ($x = 0; $x < $width; $x++) {
            $t = $tiles[$y][$x];
            if ($t === 'grass' && rand(1, 100) <= 8) {
                $tiles[$y][$x] = 'grass2';
            } elseif ($t === 'grass2' && rand(1, 100) <= 5) {
                $tiles[$y][$x] = 'grass';
            }
        }
    }
    ...
    $villagesBase = 2 + (int)round(6 * $scaleFactor);
    $villagesToPlace = max(2, min(8, $villagesBase + rand(0, 1)));
    $vCount = 0;
